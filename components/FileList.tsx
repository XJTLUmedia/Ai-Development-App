...
export const FileList: React.FC<FileListProps> = ({ files, onRemoveFile, onViewFile, disabled }) => {
  if (files.length === 0) {
    return null;
  }
  ...
  return (
    <div className="mt-4 space-y-2">
      ...
      <ul className="max-h-32 overflow-y-auto rounded-lg bg-gray-900/50 p-2">
        {files.map((filename) => (
          <li key={filename} className="flex items-center justify-between text-sm text-gray-300 py-1 px-2 rounded hover:bg-gray-700/50">
            ...
            <div className="flex items-center flex-shrink-0">
              {filename.toLowerCase().endsWith('.xlsx') && (
                <button
                  ...
                  disabled={disabled}
                  className="mr-2 text-gray-400 hover:text-blue-400 disabled:opacity-50 text-xs font-semibold"
